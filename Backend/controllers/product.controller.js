  ...
  try {
    ...
    try {
      await Inventory.create({
        productId: doc._id,
        currentStock: 0, // Start with 0 stock (low stock status)
        lowStockThreshold: 10,
        highStockThreshold: 100,
        location: "Warehouse",
        notes: `Auto-created inventory for product: ${doc.name}`
      });
    } catch (inventoryError) {
      console.error("Failed to create inventory for product:", inventoryError);
...
export async function remove(req, res, next) {
  try {
    ...
    // Also delete the corresponding inventory entry
    try {
      await Inventory.findOneAndDelete({ productId: req.params.id });
    } catch (inventoryError) {
      console.error("Failed to delete inventory for product:", inventoryError);
